...
# ============================================================
# SECCIÓN: RESULTADOS (VERSIÓN EJECUTIVA)
# ============================================================
from pathlib import Path
import numpy as np
import pandas as pd
...
# ============================================================
st.markdown(f"""
Los resultados se construyen a partir de **{int(n_sim)} escenarios simulados** a un horizonte de
**{int(horizonte)} meses**, con fecha de corte **{fecha_corte}**. El objetivo es evaluar cómo cambia
el **Ingreso Neto por Intereses (NII)** ante movimientos en los factores de riesgo del balance y,
a partir de esa distribución, estimar el **Earnings at Risk (EaR)**.
""")
...
st.markdown(f"""
<div style="background-color:#f7f7f7;padding:18px 20px;border-radius:12px;border:1px solid #e6e6e6;">
<p style="margin-bottom:10px;"><b>1.</b> El escenario base arroja un <b>NII {signo_base}</b> de <b>{fmt_mm(nii_base)}</b>, lo que indica presión sobre el margen financiero en el escenario de referencia.</p>
<p style="margin-bottom:10px;"><b>2.</b> El <b>Earnings at Risk al 99%</b> asciende a <b>{fmt_mm(ear_99)}</b>, cuantificando el deterioro potencial del NII bajo escenarios adversos extremos.</p>
<p style="margin-bottom:10px;"><b>3.</b> La mayor contribución al riesgo se concentra en la cartera <b>{cartera_top}</b> y en el factor <b>{factor_top}</b>, lo que revela una estructura de exposición claramente identificable.</p>
<p style="margin-bottom:0px;"><b>4.</b> La comparación entre el NII base y la distribución simulada permite distinguir entre el desempeño esperado del margen y su deterioro potencial en cola adversa.</p>
</div>
...
m1.metric("NII base", fmt_mm(nii_base))
m2.metric("EaR 99%", fmt_mm(ear_99))
m3.metric("Percentil 1% del NII", fmt_mm(p1_nii))
m4.metric("Media del NII simulado", fmt_mm(media_nii))

# ============================================================
# 4. VISUALES PRINCIPALES
# ============================================================
...
with col2:
    tmp_cartera = ear_cartera.sort_values("EaR 99%", ascending=True)
    fig_cartera = px.bar(
        tmp_cartera,
        x="EaR 99%",
        y="Cartera",
    ...
    fig_cartera.update_traces(texttemplate="%{text:,.0f}", textposition="outside")
    fig_cartera.update_layout(
        xaxis_title="EaR 99% (COP)",
        yaxis_title="",
        margin=dict(l=20, r=20, t=60, b=20)
    )
    st.plotly_chart(fig_cartera, use_container_width=True)
...
with col3:
    tmp_factor = ear_factor.sort_values("EaR 99%", ascending=True)
    fig_factor = px.bar(
        tmp_factor,
        x="EaR 99%",
        y="Factor",
        orientation="h",
        text="EaR 99%",
        title="EaR al 99% por factor"
    )
    fig_factor.update_traces(texttemplate="%{text:,.0f}", textposition="outside")
    fig_factor.update_layout(
        xaxis_title="EaR 99% (COP)",
        yaxis_title="",
        margin=dict(l=20, r=20, t=60, b=20)
...
st.markdown(f"""
Los resultados muestran que el balance presenta un **margen financiero base de {fmt_mm(nii_base)}**,
concentrando su mayor sensibilidad en **{cartera_top}** y en el factor **{factor_top}**. La distribución
simulada del NII permite observar que, aunque existe una dispersión relevante en los escenarios, el
riesgo extremo queda resumido en un **EaR al 99% de {fmt_mm(ear_99)}**. En términos prácticos, esto
significa que la estructura del balance es particularmente sensible a movimientos adversos en ciertas
tasas de fondeo y variables indexadoras, lo que justifica analizar el riesgo no solo de forma agregada,
sino también mediante descomposición por cartera y factor.
""")

# ============================================================
# 6. DETALLE TÉCNICO
# ============================================================
with st.expander("Ver detalle técnico de resultados", expanded=False):
    ...
    st.markdown("#### EaR por factor")
    st.dataframe(ear_factor, use_container_width=True, hide_index=True)

    st.markdown("#### EaR por cartera")
    st.dataframe(ear_cartera, use_container_width=True, hide_index=True)

    st.markdown("#### NII base mensual")
    st.dataframe(nii_base_mensual, use_container_width=True, hide_index=True)

    st.markdown("#### Posiciones con mayor aporte al NII base")
    detalle_base_orden = detalle_base.sort_values("NII", ascending=False).reset_index(drop=True)

    cpos, cneg = st.columns(2)

    with cpos:
        st.markdown("**Top 10 aportes positivos**")
        st.dataframe(detalle_base_orden.head(10), use_container_width=True, hide_index=True)

    with cneg:
        st.markdown("**Top 10 aportes negativos**")
        st.dataframe(
            detalle_base_orden.sort_values("NII", ascending=True).head(10),
            use_container_width=True,
            hide_index=True
